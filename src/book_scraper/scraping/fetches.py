...
def save_html(path: Path, filename: str, content: str) -> None:
...
def fetch_one(entry: Dict, fetcher: Fetcher, path: Path, cancel_event):
    ...
    save_html(path, filename, html_content)
    logger.info("Fetched and saved %s", url)
    time.sleep(SLEEP_BETWEEN_REQUESTS)


def fetch_urls_from_mapping(
    mapping_path: Path, store_path: Path, max_failures: int
) -> None:
    ...
    with Fetcher() as fetcher:
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executer:
            future_to_url = {
                executer.submit(
                    fetch_one, entry, fetcher, store_path, cancel_event
                ): entry
